  ...
  <div class="container">
    ...
    >
      <el-form-item label="用户名" prop="username">
        <el-input size="medium" clearable v-model="form.username" :disabled="isEdited" />
      </el-form-item>
      <el-form-item label="邮箱" prop="email">
        <el-input size="medium" clearable v-model="form.email" auto-complete="new-password" />
      </el-form-item>
      ...
      <el-form-item v-if="pageType === 'add'" label="确认密码" prop="confirm_password" label-position="top">
        <el-input size="medium" clearable type="password" v-model="form.confirm_password" autocomplete="off" />
      </el-form-item>
      ...
      <el-form-item v-if="form.group_id === 1" label="查询范围" prop="query_scope" :span="25">
        <el-cascader
          v-model="form.query_scope"
          :options="proxyList"
...
export default {
  ...
  data() {
    ...
    const validateQueryScope = (rule, value, callback) => {
      if (this.form.group_id !== 1) {
        callback()
      }
      if (value.length === 0) {
        callback(new Error('代理分组下的用户需指定订单可查询范围'))
      }
      callback()
    }
    return {
      ...
      rules: {
        ...
        query_scope: [{ validator: validateQueryScope, trigger: 'blur', required: true }],
      },
    }
  },
  methods: {
        ...
        if (valid) {
          ...
          if (this.pageType === 'add') {
            ...
            if (
              ...
              && JSON.stringify(this.form.query_scope) === JSON.stringify(this.info.query_scope)
            ) {
            ...
            try {
              this.loading = true
              if (JSON.stringify(this.form.query_scope) !== JSON.stringify(this.info.query_scope)) {
                res = await Admin.updateUserQueryScope(this.form.query_scope, this.id)
              }
    ...
    setInfo() {
      ...
      this.form.email = this.info.email
      this.form.group_id = this.info.group_id
      this.form.query_scope = this.info.query_scope
    },
  },
  watch: {
    groups: {
    ...
    proxys: {
      handler() {
        this.proxyList = optionModel.generateProxyOptions(this.proxys)
      },
      immediate: true,
    },
  },
